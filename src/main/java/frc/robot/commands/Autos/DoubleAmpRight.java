...
public class DoubleAmpRight extends SequentialCommandGroup {
  /** Creates a new AutoTwo. */
  public DoubleAmpRight(Elevator objElevator, TilterV2 objTilter, SwerveSubsystem objSwerve, Feeder objFeeder, Shooter objShooter, Intake objIntake, boolean bFwd) {
    // Add your commands in the addCommands() call, e.g.
    // addCommands(new FooCommand(), new BarCommand());
    addCommands(
      new PositionForSpeaker(objElevator, objTilter),
      new ManualFeedNShoot(objFeeder, objShooter, 4500.0, 0.3, 0.6).withTimeout(1.5),
      new ParallelCommandGroup(
        new AutoDrive(objSwerve, -0.75, 0.15, -0.28),
      ...
      new ParallelCommandGroup( 
        new AutoDrive(objSwerve, 0.75, -0.1, 0.28),
        new IntakeNFeederRun(objElevator, objTilter, objFeeder, objIntake, objShooter, bFwd).withTimeout(1.0)
      ).withTimeout(4.3),
      new PositionForSpeaker(objElevator, objTilter).withTimeout(3.0),
